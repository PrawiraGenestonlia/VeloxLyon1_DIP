...
#include "SparkFun_I2C_GPS_Arduino_Library-modified.h" //Use Library Manager or download here: https://github.com/sparkfun/SparkFun_I2C_GPS_Arduino_Library
...
void setup()
{
  ...
  SerialUSB.println("GTOP Read Example");
  ...
  SerialUSB.println("GPS module found!");
}
...
void loop()
{
  while (myI2CGPS.available()) //available() returns the number of new bytes available from the GPS module
  {
    ...
    if(incoming == '$') SerialUSB.println(); //Break the sentences onto new lines
    SerialUSB.write(incoming); //Print this character
  }
}
